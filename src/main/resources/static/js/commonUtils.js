        ...
        <div id="customAlert" class="modal-overlay">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button class="modal-button" id="btnCancel"  onclick="closeModal(false)">취소</button>
                    <button class="modal-button" onclick="closeModal(true)">확인</button>
                </div>
            </div>
        </div>`;
...
let modalResolve = null; // Promise 처리를 위한 변수

function myAlert(message) {
    injectModal();
    document.getElementById('modalMsg').innerText = message;
    document.getElementById('btnCancel').style.display = 'none'; // 취소 버튼 숨김
    document.getElementById('customAlert').style.display = 'flex';
    // 사용자의 선택을 기다리는 Promise 반환
...
function myConfirm(message) {
    injectModal();
    document.getElementById('modalMsg').innerText = message;
    document.getElementById('btnCancel').style.display = 'inline-block'; // 취소 버튼 보임
    document.getElementById('customAlert').style.display = 'flex';
...
function closeModal(result) {
    ...
    if (modalResolve) {
        modalResolve(result); // 확인이면 true, 취소면 false 반환
        modalResolve = null;
    }
}
